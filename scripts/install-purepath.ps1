...
$Asset = "purepath-windows-x64.exe"
...
if ([System.Runtime.InteropServices.RuntimeInformation]::OSArchitecture -ne "X64") {
    throw "The current PurePath release supports only Windows x64."
}
if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
    throw "GitHub CLI is required to download this private repository's release assets."
}
if (-not $InstallDirectory) {
    $InstallDirectory = Join-Path $env:LOCALAPPDATA "Programs\PurePath"
}

$TemporaryDirectory = Join-Path ([System.IO.Path]::GetTempPath()) (
    "purepath-install-{0}-{1}" -f $PID, [guid]::NewGuid().ToString("N")
)
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "GitHub CLI could not download the PurePath release."
    }
    ...
    Write-Output "Installed PurePath to $InstallDirectory\purepath.exe"
